...
// ตรวจสอบเวลาว่าอยู่ในช่วง 06:30 ถึง 17:30 หรือไม่
...
// เช็ค userPermits หรือ employee ที่มี code
...
void loop() {
  ...
  Firebase.setString(firebaseData, "/logs/" + uniqueKey + "/category", table);
            if (table == "visitorRegisters" || table == "previsitorRegisters") {
            String activePath = "/" + table + "/" + key + "/active";
            if (Firebase.setBool(firebaseData, activePath, false)) {
              Serial.println("🔒 Visitor marked as exited (active = false)");
            } else {
              Serial.println("❌ Failed to update 'active' flag");
            }
          }
